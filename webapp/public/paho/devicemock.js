(function () {
  ...
  let client = new Paho.MQTT.Client(
    "localhost",          // host
    Number(3000),         // port
    "403539"              // client id, must be unique
  )
  ...
  willMessage.destinationName = "status/403539"
  ...
  function onConnect() {
    ...
    setInterval(createreading, 5000);

    let message = new Paho.MQTT.Message("Online")
    message.destinationName = "status/403539"
  ...
  function createreading() {
    ...
    let t = Math.round(1900 + Math.random() * 100)
    let h = Math.round(76000 + Math.random() * 1000)
    let p = Math.round(1015230 + Math.random() * 5)

    let message = new Paho.MQTT.Message(t.toString())
    message.destinationName = "temperature/403539"
    message.retained = true
    client.send(message)

    message = new Paho.MQTT.Message(h.toString())
    message.destinationName = "humidity/403539"
    message.retained = true
    client.send(message)

    message = new Paho.MQTT.Message(p.toString())
    message.destinationName = "pressure/403539"
    message.retained = true
    client.send(message)

    message = new Paho.MQTT.Message(`{"t":${t},"h":${h},"p":${p}}`)
    message.destinationName = "readings/403539"
    message.retained = true 
    client.send(message)
  }
